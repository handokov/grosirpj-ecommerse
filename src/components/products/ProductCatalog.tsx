...
export default function ProductCatalog() {
  ...
  const [filterCategory, setFilterCategory] = useState<string>(selectedCategoryId || 'all');
  ...
  }, [searchQuery, filterCategory, sort, page]);
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <form onSubmit={handleSearch} className="flex-1 flex gap-2">
            ...
            <Button type="submit" variant="default" className="rounded-xl bg-emerald-600 hover:bg-emerald-700">
              Cari
            </Button>
          </form>
          ...
          <div className="flex gap-2">
            ...
            <div className="hidden sm:flex border rounded-xl overflow-hidden">
              ...
              <Button
                ...
                size="icon"
                className={`rounded-none ${viewMode === 'list' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}`}
        ...
        <div className={`${showFilters ? 'block' : 'hidden'} sm:block mb-6`}>
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              size="sm"
              className={`rounded-full ${filterCategory === 'all' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}`}
            ...
            {categories.map((cat) => (
              <Button
                ...
                size="sm"
                className={`rounded-full ${filterCategory === cat.id ? 'bg-emerald-600 hover:bg-emerald-700' : ''}`}
          ...
          <div className={`grid ${viewMode === 'grid' ? 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4' : 'grid-cols-1'} gap-4`}>
            {Array.from({ length: 8 }).map((_, i) => (
              <Card key={i} className="overflow-hidden">
                <div className={`${viewMode === 'grid' ? 'aspect-square' : 'h-32'} bg-gray-200 animate-pulse`} />
        ...
        {totalProducts > 20 && (
          <div className="flex items-center justify-center gap-2 mt-8">
            ...
            <span className="text-sm text-muted-foreground">
              Hal {page} dari {Math.ceil(totalProducts / 20)}
            </span>
            <Button
              variant="outline"
              className="rounded-xl"
